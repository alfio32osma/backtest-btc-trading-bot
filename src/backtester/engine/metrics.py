...
def print_quant_report(metrics: Dict[str, Any], final_state: Any) -> None:
    #print a professional backtest report."""
    try:
        ...
        print(f"SORTINO RATIO (Ann.):     {metrics['sortino_ratio']:.2f}")
        print("-" * 70)
        print(f"WIN RATE:                 {metrics['win_rate_pct']:.2f}%")
        print(f"PROFIT FACTOR:            {metrics['profit_factor'] if metrics['profit_factor'] != float('inf') else 'INF':.2f}")
        print(f"TOTAL TRADES:             {metrics['total_trades']}")
        print(f"SKIPPED TRADES (Pauses):  {getattr(final_state, 'skipped_trades', 0)}")
