...
def ejecutar_backtest_con_proteccion(csv_path):
    ...
    # 2. RESAMPLE 4H
    col_close = next((c for c in ['close', 'last', 'price'] if c in df.columns), df.columns[1])
    col_high  = next((c for c in ['high', 'max'] if c in df.columns), col_close)
    col_low   = next((c for c in ['low', 'min'] if c in df.columns), col_close)

    df_h = df.resample('4h', on='date').agg({
        col_close: 'last',
        col_high:  'max',
        col_low:   'min'
    }).dropna()
    df_h.columns = ['close', 'high', 'low']
