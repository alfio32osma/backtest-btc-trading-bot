...
def test_sharpe_and_sortino_use_configured_timeframe():
    ...
    expected_ratio = math.sqrt((365 * 24) / 4.0) / math.sqrt((365 * 24) / 4.0 / 1.0)
    assert metrics_1h["sharpe_ratio"] != metrics_4h["sharpe_ratio"]
    assert abs(metrics_1h["sharpe_ratio"] / metrics_4h["sharpe_ratio"] - math.sqrt(8760 / 2190)) < 1e-6
    assert abs(metrics_1h["sortino_ratio"] / metrics_4h["sortino_ratio"] - math.sqrt(8760 / 2190)) < 1e-6
